...
export const getPostBySlug = async (c: Context) => {
  const post = await db.select().from(Blogs).where(eq(Blogs.slug, c.req.param('slug'))).get();
  if (!post) {
...
export const createPost = async (c: Context) => {
  ...
  const existingPost = await db.select().from(Blogs).where(eq(Blogs.slug, slug)).get();
...
export const deletePost = async (c: Context) => {
  ...
  await db.delete(Blogs).where(eq(Blogs.slug, c.req.param('slug'))).run();
  return c.json({post}, 200);
}